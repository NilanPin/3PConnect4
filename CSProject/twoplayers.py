...
def winnext(board,piece):
    valid = AvailablePos(board)
    for col in valid:
        row = nextrow(board, col)
        new_board = board.copy()
        droppiece(new_board, row, col, piece)
        if winningpos(board,piece) == True:
            return True, col
...
while not game_over:
    ...
    if turn == 2 and not game_over:
        if winnext(board, AI):
            col = winnext(board,AI)
        else:
            col, minimax_score = minimax(board, 5, -math.inf, math.inf, 2)
        if isvalid(board, col):
            row = nextrow(board, col)
